...
// html 파일이 아니라 data 를 리턴해주는 컨트롤러 = RestController
@RestController
public class DummyControllerTest {
    ...
    // save 함수는 id 를 전달하면 해당 id 에 대한 데이터가 없으면 insert 를 해줌
    // email, password

    @Transactional  // save 를 사용하지 않아도 업데이트가 됨
    @PutMapping("/dummy/user/{id}")
    public User updateUser(@PathVariable int id, @RequestBody User requestUser){    // json 데이터를 요청 -> JAVA 오브젝트( MessageConverter 의 Jackson 라이브러리가 변환해서 받아줌, 이때 필요한 어노테이션이 @RequestBody )
        ...
        //userRepository.save(requestUser);   // 원래 save 는 Insert 할 때 사용

        // 더티 체킹 = 찌꺼기 같은거 찾아서 날리는 거
        return user;
    }
   ...
   /* @PostMapping("/dummy/join")
    public  String join(String username, String password, String email){    // key=value 형태로 받음 (약속된 규칙)
    ...
    @PostMapping("/dummy/join")
    public  String join(User user){    // key=value 형태로 받음 (약속된 규칙)
